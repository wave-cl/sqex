//! SIP-60 §At a former home: devices of an account that moved. Alice registered a phone and
...
//! and its own stale pair only when H cannot be asked. SIP-60 §Saying whose list it is: asked with
...
/// SIP-60 §Saying whose list it is: the list and whose it is.
...
async fn a_former_home_that_cannot_ask_the_home_answers_its_own_registry() {
    ...
    // SIP-60 §Saying whose list it is: and says so, to a caller that asks.
    assert_eq!(
